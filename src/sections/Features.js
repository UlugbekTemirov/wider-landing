...
const Features = ({ id }) => {
  ...
  return (
    <section id={id} className="md:py-[50px] py-10">
      <Container>
        <h1 className="text-center text-[#6941C6] md:text-[16px] font-semibold">
          Features
        </h1>
        ...
        <div className="grid xl:grid-cols-4 md:grid-cols-2 grid-cols-1 gap-[24px] md:mt-[64px] mt-10">
          {features.map(({ icon: Icon, title, description, link }, index) => (
            <div className="bg-[#F9FAFB] p-[24px] rounded-[10px]" key={index}>
              <span className="bg-[#7F56D9] w-[48px] h-[48px] flex items-center justify-center rounded-[10px]">
              ...
              <a
                href={link}
                className="flex items-center w-fit text-[#6941C6] text-[16px] gap-2 font-semibold mt-5 hover:gap-4 duration-200"
              >
                View all
                <FaArrowRightLong />
              </a>
